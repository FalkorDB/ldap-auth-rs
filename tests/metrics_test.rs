...
async fn test_metrics_endpoint_exists() {
    ...
    assert!(body.contains("# TYPE"), "Response should contain Prometheus TYPE declarations");
    
    // Verify axum-prometheus HTTP metrics are present
    assert!(body.contains("axum_http_requests_pending"), "Should include HTTP metrics from axum-prometheus");
    
    println!("✅ Metrics endpoint working! Found {} lines of metrics", body.lines().count());
}
...
async fn test_metrics_track_http_requests() {
    ...
    assert!(body.contains("http_requests_total"), "Should track HTTP request count");
    assert!(body.contains("http_requests_duration_seconds"), "Should track request duration");
    
    // Verify the health endpoint was recorded
    assert!(body.contains("/health"), "Should record /health endpoint in metrics");
}
