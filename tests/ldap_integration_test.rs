use ldap_auth_rs::{
    config::Config, db::DbService, ldap::LdapServer, models::UserCreate, redis_db::RedisDbService, tls,
};
use std::sync::Arc;
...
async fn start_ldap_server_with_tls(db: Arc<dyn DbService>, port: u16) -> Result<(String, String), String> {
    let addr = format!("127.0.0.1:{}", port);
    let base_dn = "dc=example,dc=com".to_string();
    let server = LdapServer::new(db, base_dn);
    
    // Generate test certificates
    let (cert_path, key_path) = tls::generate_test_certs()
        .map_err(|e| format!("Failed to generate test certs: {}", e))?;
...
async fn test_ldap_bind_success() {
    ...
    db.create_user(user_create).await.expect("Failed to create user");
    ...
    stream.write_all(&bind_request).await.expect("Failed to send bind");
    
    let mut response = vec![0u8; 1024];
    let n = stream.read(&mut response).await.expect("Failed to read response");
...
async fn test_ldap_search_authenticated() {
    ...
    let mut response = vec![0u8; 1024];
    let _ = stream.read(&mut response).await.expect("Failed to read bind response");
    
    // Now search
    let search_request = create_search_request(2, "dc=example,dc=com");
    stream.write_all(&search_request).await.expect("Failed to send search");
    
    let mut search_response = vec![0u8; 4096];
    let n = stream.read(&mut search_response).await.expect("Failed to read search response");
    let search_response = &search_response[..n];
    
    assert!(!search_response.is_empty(), "Should receive search response");
...
async fn test_ldap_whoami() {
    ...
    stream.write_all(&whoami_request).await.expect("Failed to send whoami");
    
    let mut whoami_response = vec![0u8; 1024];
    let n = stream.read(&mut whoami_response).await.expect("Failed to read whoami response");
    let whoami_response = &whoami_response[..n];
    
    assert!(!whoami_response.is_empty(), "Should receive whoami response");
    ...
    assert!(response_str.contains("dn:"), "Should contain DN in response");
...
async fn test_ldap_unbind() {
    ...
    stream.write_all(&unbind_request).await.expect("Failed to send unbind");
...
async fn test_ldap_multiple_operations() {
    ...
    let mut response = vec![0u8; 1024];
    let n = stream.read(&mut response).await.expect("Failed to read bind response");
    ...
    let mut whoami_response = vec![0u8; 1024];
    let n = stream.read(&mut whoami_response).await.expect("Failed to read whoami");
    ...
    let mut search_response = vec![0u8; 4096];
    let n = stream.read(&mut search_response).await.expect("Failed to read search");
...
async fn test_ldap_bind_success_tls() {
    ...
    for cert in certs {
        root_store.add(&rustls::Certificate(cert)).expect("Failed to add cert to root store");
    }
    ...
    tls_stream.write_all(&bind_request).await.expect("Failed to send bind");
    
    let mut response = vec![0u8; 1024];
    let n = tls_stream.read(&mut response).await.expect("Failed to read response");
    let response = &response[..n];
...
async fn test_ldap_search_authenticated_tls() {
    ...
    let mut response = vec![0u8; 1024];
    let _ = tls_stream.read(&mut response).await.expect("Failed to read bind response");
    
    // Now search over TLS
    let search_request = create_search_request(2, "dc=example,dc=com");
    tls_stream.write_all(&search_request).await.expect("Failed to send search");
    
    let mut search_response = vec![0u8; 4096];
    let n = tls_stream.read(&mut search_response).await.expect("Failed to read search response");
    let search_response = &search_response[..n];
...
async fn test_ldap_invalid_credentials_tls() {
    ...
    let result_code = parse_bind_response(response).expect("Failed to parse response");
    assert_eq!(result_code, 49, "TLS bind should fail with invalid credentials");
