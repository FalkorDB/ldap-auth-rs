...
impl DbService for RedisDbService {
    async fn create_user(&self, user: UserCreate) -> Result<User> {
        ...
        conn.sadd::<_, _, ()>(&org_users_key, &user.username).await?;
    ...
    async fn delete_user(&self, organization: &str, username: &str) -> Result<()> {
        ...
        for group in groups {
            let _ = self.remove_user_from_group(organization, &group.name, username).await;
        }
...
mod tests {
    ...
    fn test_key_generation() {
        assert_eq!(
            RedisDbService::user_key("acme", "john"),
            "user:acme:john"
        );
        assert_eq!(
            RedisDbService::group_key("acme", "devs"),
            "group:acme:devs"
        );
        assert_eq!(
            RedisDbService::org_users_key("acme"),
            "org:acme:users"
        );
        assert_eq!(
            RedisDbService::org_groups_key("acme"),
            "org:acme:groups"
        );
    }
}
